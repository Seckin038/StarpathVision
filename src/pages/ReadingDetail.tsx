...
import PersonalizationNotice from "@/components/PersonalizationNotice"; // Import the new component
...
function mapSpreadIdToKind(id: string | null): SpreadKind {
  ...
  const kindMap: Record<string, SpreadKind> = {
    ...
    "line-3": "line-3", "star-6": "star-6", "horseshoe-7": "horseshoe-7",
    "celtic-cross-10": "cross-10", "year-12": "year-12",
  };
...
export default function ReadingDetailPage() {
  ...
  const downloadPDF = useCallback(async () => {
    ...
    const canvas = await html2canvas(element, { backgroundColor: "#0a090c", scale: 2, useCORS: true, windowWidth: element.scrollWidth, windowHeight: element.scrollHeight });
  ...
  const cards = useMemo(() => {
    ...
    return src.map((c: any, idx: number) => ({
      id: c.id ?? `c-${idx}`, name: c.name ?? `Card ${idx + 1}`,
      imageUrl: c.imageUrl ?? (c.image ? `/tarot/${c.image}` : undefined),
      upright: c.upright ?? !c.isReversed,
    }));
  }, [r]);

  if (loading) return <div className="min-h-screen grid place-items-center"><Loader2 className="h-7 w-7 animate-spin text-amber-500" /></div>;
  if (!r) return <div className="p-8 text-center text-red-400">{t('readingDetail.notFound')}</div>;
      ...
      <div ref={printRef} className="space-y-6">
        <Card className="bg-stone-950/60 border-white/10">
          ...
          <CardContent className="space-y-6">
            <PersonalizationNotice context={r.payload?.userContextSnapshot} />
            {isTarot && cards.length > 0 ? (
              <TarotSpreadBoard cards={cards} kind={mapSpreadIdToKind(r.spread_id)} cardsFlipped={true} />
            ) : (
            ...
            {r.interpretation.story ? (
              <TarotInterpretationPanel
                items={cards.map((c: any, i: number) => ({
                  index: i + 1, name: c.name, imageUrl: c.imageUrl, upright: !!c.upright,
