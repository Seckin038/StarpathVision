...
function circlePositions(n: number, opts?: { cx?: number; cy?: number; r?: number; startDeg?: number }) {
  ...
  const cy = opts?.cy ?? 0.5; // Corrected to be perfectly centered
  const r  = opts?.r  ?? 0.36;
  ...
  return (
    ...
    >
      {cards.map((card, i) => {
        ...
        const cardAspectRatio = 65 / 112; // Correct aspect ratio

        const cardW = widthPct;
        const cardH = cardW * (boardAspectRatio / cardAspectRatio);
